...
// O rasgo da placa desce (wire_seal_depth) dentro desse chão → sobra (bottom_bar_height - wire_seal_depth) de parede sólida no fundo.
bottom_bar_height = 6.0;
...
wire_hole_d = 3;
// Profundidade da cava na base do furo (para aplicar silicone de aquário e isolar o cobre)
wire_seal_depth = 5.0;
...
module frame() {
  difference() {
    ...
    // Cava inferior da fenda (desce wire_seal_depth no bottom_bar — aplique silicone aqui para isolar o cobre)
    translate([0, (gap_between_plates / 2) + (slot_w / 2), bottom_bar_height - wire_seal_depth / 2 + 0.5])
      cube([plate_width + tolerance, slot_w, wire_seal_depth + 1], center=true);
    ...
    //
    // Cada furo tem uma cava de wire_seal_depth na base: insira o cobre e vede com silicone de aquário.

    // Furo Esquerdo (Placa Frontal) — cava de silicone + canal até o rasgo
    translate([-plate_width / 3, (gap_between_plates / 2) + (slot_w / 2), -1])
      cylinder(h=wire_seal_depth + 1, d=wire_hole_d);
    translate([-plate_width / 3, (gap_between_plates / 2) + (slot_w / 2), wire_seal_depth - 1])
      cylinder(h=bottom_bar_height - wire_seal_depth + 3.0, d=wire_hole_d);

    // Furo Direito (Placa Traseira) — cava de silicone + canal até o rasgo
    translate([plate_width / 3, -(gap_between_plates / 2) - (slot_w / 2), -1])
      cylinder(h=wire_seal_depth + 1, d=wire_hole_d);
    translate([plate_width / 3, -(gap_between_plates / 2) - (slot_w / 2), wire_seal_depth - 1])
      cylinder(h=bottom_bar_height - wire_seal_depth + 3.0, d=wire_hole_d);
